...
class machine_machine_transfer(models.Model):
    ...
    is_ribbon=fields.Boolean(default=False)
    ...
    def _compute_alternate_machine_ids(self):
        ...
        for rec in self:
            if rec.transfer_type_test=='install':
                machine=rec.machine_name_test.search([('status','=','active')])
                print("machine", machine)
                print("machine_ids", machine.ids)
            elif rec.transfer_type_test=='remove':
                machine=rec.machine_name_test.search([('status','=','inservice')])
                print("machine", machine)
                print("machine_ids", machine.ids)
            else:
